...
class TestUsers:
    ...
    async def test_get_users_admin(self, client, test_admin_user):
    ...
    async def test_get_users_non_admin(self, client, test_user):
    ...
    async def test_get_user_by_id_unauthorized(self, client, test_user):
    ...
    async def test_update_user_unauthorized(self, client, test_user):
        """Test updating user without token"""
        response = await client.put(
            f"/users/{test_user.id}", json={"full_name": "Updated Name"}
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
