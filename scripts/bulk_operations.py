...
async def bulk_insert_raw_sql(record_count: int = 1000):
    ...
    values_data = []
    for i in range(record_count):
        values_data.append(f"('Product {i:06d}', 'Description for product {i}', {10.0 + (i % 1000)}, 100, {(i % 3) + 1}, true)")
...
async def test_bulk_update():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(
            text("UPDATE products SET stock_quantity = stock_quantity + 10 WHERE price > 500")
        )
        await session.commit()
...
async def test_bulk_delete():
    ...
    async with AsyncSessionLocal() as session:
        ...
        await session.execute(
            text("DELETE FROM products WHERE price < 50")
        )
        await session.commit()
...
async def run_insert_tests():
    ...
    for size in test_sizes:
        ...
        for approach_func in approaches:
            ...
            records_per_sec = round(size / (result['time_ms'] / 1000), 2) if result['time_ms'] > 0 else 0
            print(f"{result['approach']:<20} {result['queries']:<8} {result['time_ms']:<10} {records_per_sec:<12}")
