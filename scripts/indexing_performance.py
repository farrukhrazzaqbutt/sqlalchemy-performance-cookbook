...
from sqlalchemy import Index, and_, func, or_, select, text
...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite+aiosqlite:///./indexing_test.db")
...
async def query_users_by_email_and_status(session):
    ...
    result = await session.execute(
        select(User)
        .where(and_(User.email.like("%user1%"), User.is_active == True))
        .limit(100)
    )
...
async def query_orders_by_user_and_status(session):
    ...
    result = await session.execute(
        select(Order)
        .where(and_(Order.user_id == 1000, Order.status == "completed"))
        .limit(100)
    )
    return result.scalars().all()
...
async def query_products_with_joins(session):
    ...
    result = await session.execute(
        ...
        .join(Category)
        .where(and_(Product.is_active == True, Category.name == "Electronics"))
        .limit(100)
    )
    return result.all()
...
async def run_performance_tests():
    ...
    async with AsyncSessionLocal() as session:
        for query_name, query_func in test_queries:
            ...
            print(
                f"{result['query']:<30} {result['queries']:<8} {result['time_ms']:<10} {result['result_count']:<8}"
            )
