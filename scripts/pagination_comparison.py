    ...
    async with AsyncSessionLocal() as session:
        ...
        for i in range(5000):
            orders.append(Order(
                ...
                created_at=Base.metadata.tables['orders'].c.created_at.default.arg() if hasattr(Base.metadata.tables['orders'].c.created_at.default, 'arg') else None
            ))
...
async def offset_limit_pagination(page: int, page_size: int = 20):
    ...
    async with AsyncSessionLocal() as session:
        # Count total records
        count_result = await session.execute(select(text("COUNT(*)")).select_from(Order))
...
async def test_pagination_performance():
    ...
    print(f"{'Approach':<15} {'Page/Cursor':<12} {'Queries':<8} {'Time (ms)':<10} {'Records':<8}")
    print("-" * 80)
    ...
    for page in test_pages:
        ...
        print(f"{result['approach']:<15} {result['page']:<12} {result['queries']:<8} {result['time_ms']:<10} {result['records_returned']:<8}")
    ...
    for i, page in enumerate(test_pages):
        ...
        results.append(result)
        print(f"{result['approach']:<15} {result['cursor'] or 'None':<12} {result['queries']:<8} {result['time_ms']:<10} {result['records_returned']:<8}")
...
async def test_large_offset_performance():
    ...
    print(f"{'Approach':<15} {'Page':<8} {'Queries':<8} {'Time (ms)':<10} {'Records':<8}")
    print("-" * 80)
    
    for page in large_pages:
        result = await offset_limit_pagination(page)
        print(f"{result['approach']:<15} {result['page']:<8} {result['queries']:<8} {result['time_ms']:<10} {result['records_returned']:<8}")
